...
public interface test1<一般商品> {
    ...
    @GET("normolgood?api_key=keyq0EkjYQRAzfrUN")    // 設置一個GET連線，路徑為albums/1
    Call<shopcar> getShopcar();    // 取得的回傳資料用Albums物件接收，連線名稱取為getAlbums

    @GET("normolgoo?api_key=keyq0EkjYQRAzfrUN")    // 設置一個GET連線，路徑為albums/1
    Call<checkout1> getCheck1();    // 取得的回傳資料用Albums物件接收，連線名稱取為getAlbums

    @GET("normolgoo/{id}") // 用{}表示路徑參數，@Path會將參數帶入至該位置
