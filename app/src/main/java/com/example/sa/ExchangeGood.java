package com.example.sa;

import java.util.List;

public class ExchangeGood {


    private List<String> seller_account;
    private String change_product_name;
    private String change_product_narrative;
    private int change_product_price;
    private String change_color;
    private String change_size;
    private List<String> change_transport_method;
    private String change_area;
    private List<String> membername;
    private List<String> change_product_class;
    private String range_you_want_to_change;



    private List<String> productclass;

    public List<String> getSeller_account() {
        return seller_account;
    }

    public String getChange_product_name() {
        return change_product_name;
    }

    public String getChange_product_narrative() {
        return change_product_narrative;
    }

    public int getChange_product_price() { return change_product_price; }

    public String getChange_color() {
        return change_color;
    }

    public String getChange_size() {
        return change_size;
    }

    public List<String> getChange_transport_method() {
        return change_transport_method;
    }

    public String getChange_area() { return change_area; }

    public List<String> getMembername() { return membername; }

    public List<String> getChange_product_class() { return change_product_class; }

    public String getRange_you_want_to_change() { return range_you_want_to_change; }

    public List<String> getProductclass() { return productclass; }




}
